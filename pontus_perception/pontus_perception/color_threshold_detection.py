...
class ThresholdDetection:
    ...
    # String label to identify the specific object
    label: str

    # Debug bounding box settings
    debug_color_bgr: Tuple[int, int, int] = (255, 0, 0)
    thickness: int = 2
...
def get_bounding_boxes(contours: List[np.ndarray], 
                    color_bgr = (255, 0, 0), 
                    line_thickness=2,
                    contour_area_threshold=150) -> List[ThresholdDetection]:
    """
    ...
    """
    bounding_boxes: List[ThresholdDetection] = []

    for contour in contours:

        if cv2.contourArea(contour) > contour_area_threshold:
            x, y, w, h = cv2.boundingRect(contour)
            bounding_boxes.append(ThresholdDetection(x, y, w, h,
...
def display_bounding_boxes(image: np.ndarray, 
                           bounding_boxes: List[ThresholdDetection]) -> np.ndarray:
    """
