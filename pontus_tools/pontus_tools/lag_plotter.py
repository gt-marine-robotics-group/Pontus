...
class LagPlotter(Node):
    ...
    def __init__(self) -> None:
        ...
        self.offset_series: Dict[str, LagSeries] = {
            ...
            'odom_pointcloud': self._make_series('Odom - clustercloud', 'tab:orange'),
            'yolo_camera': self._make_series('YOLO - camera', 'tab:cyan'),  # NEW
        }
    ...
    def _make_series(self, label, color, linestyle='-', linewidth=3.0):
        return LagSeries(label, color, deque(maxlen=self.history_size),
                         deque(maxlen=self.history_size), linestyle, linewidth)

    def _create_subscriptions(self):
        qos = QoSProfile(depth=10)
        self.create_subscription(CompressedImage, self.camera_topic, self.camera_callback, qos)
        self.create_subscription(Detection2DArray, self.yolo_topic, self.yolo_callback, qos)
        self.create_subscription(PointCloud2, self.pointcloud_topic, self.pointcloud_callback, qos)
        self.create_subscription(Odometry, self.odom_topic, self.odom_callback, qos)

    def _create_plot(self):
        plt.ion()
        self.figure, self.offset_ax = plt.subplots(figsize=(12, 8))

        self.offset_lines = {}
        for key, s in self.offset_series.items():
            line, = self.offset_ax.plot([], [], label=s.label, color=s.color)
            self.offset_lines[key] = line

        self.offset_ax.legend()
        self.offset_ax.grid(True)

        self.stats_text = self.figure.text(0.01, 0.01, '', family='monospace')
    ...
    def _record_series(self, s, t, v):
        s.times_sec.append(t)
        s.values_ms.append(v)

    def camera_callback(self, msg):
        self.last_camera_stamp_sec = self._stamp_to_seconds(msg.header.stamp)

    def pointcloud_callback(self, msg):
        self.last_pointcloud_stamp_sec = self._stamp_to_seconds(msg.header.stamp)

    def odom_callback(self, msg):
        self.last_odom_stamp_sec = self._stamp_to_seconds(msg.header.stamp)
    ...
    def yolo_callback(self, msg):
        ...
        if self.last_pointcloud_stamp_sec:
            self._record_series(
                self.offset_series['yolo_pointcloud'],
                now,
                (yolo_stamp - self.last_pointcloud_stamp_sec) * 1000,
            )

        if self.last_camera_stamp_sec:
            self._record_series(
                self.offset_series['yolo_camera'],
                now,
                (yolo_stamp - self.last_camera_stamp_sec) * 1000,
            )

    def update_plot(self):
        t = self._now_seconds() - self.start_time_sec

        for key, s in self.offset_series.items():
            x = [ts - self.start_time_sec for ts in s.times_sec]
            y = list(s.values_ms)
            self.offset_lines[key].set_data(x, y)

        self.offset_ax.relim()
        self.offset_ax.autoscale()

        stats = []
        if self.offset_series['yolo_camera'].values_ms:
            val = self.offset_series['yolo_camera'].values_ms[-1]
            stats.append(f'YOLO - Camera: {val:.1f} ms')
