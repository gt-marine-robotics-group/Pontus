...
BlueViewSonar::BlueViewSonar()
    : head_(NULL), latest_ping_(NULL), mag_img_(NULL), cur_ping_(0), has_ping_(false), has_image_(false), has_init_(false) {
}
...
void BlueViewSonar::generateImage() {
    ...
    mag_img_ = image_generator_.GetImageRTheta(latest_ping_);
    has_image_ = true;
}
