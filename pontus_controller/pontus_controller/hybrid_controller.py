...
class HybridControllerNode(Node):
    ...
    def odometry_callback(self, msg):
        ...
        msg.angular.x = self.pid_angular[0](-roll, self.get_clock().now() - self.prev_time)
        msg.angular.y = self.pid_angular[1](-pitch, self.get_clock().now() - self.prev_time)
        '''
        ...
        accel_msg.linear.x = 0.75 * self.cmd_linear[0]
        accel_msg.linear.y = 3.0 * self.cmd_linear[1]

        # Maintain depth if no commanded velocity
        if abs(self.cmd_linear[2]) < 0.05 and self.cmd_depth != None:
