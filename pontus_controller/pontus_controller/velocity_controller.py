...
class VelocityNode(Node):
    ...
    def calculate_feedforward(self, odom: Odometry) -> tuple[np.ndarray, np.ndarray]:
        ...
        world_acceleration_buoyancy = np.array(
            [0.0, 0.0, f_net / self.vehicle_params.mass])
        ...
        angular_f_drag = np.copysign(
            C[3:] * self.cmd_angular ** 2, self.cmd_angular)

        # Compute Feed Forward Terms
        linear_ff = -body_acceleration_buoyancy + \
