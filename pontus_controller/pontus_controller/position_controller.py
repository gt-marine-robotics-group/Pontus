...
class PositionController(Node):
    ...
    def is_pose_complete(self) -> bool:
        ...
        depth_good = np.abs(goal_linear_err[2]) < self.depth_threshold
        orientation_good = self.skip_orientation or np.all(np.abs(goal_angular_err) < self.angular_thresholds)
        velocity_good = np.all(np.abs(linear_vel) < self.velocity_thresholds[0]) \
                           and np.all(np.abs(angular_vel) < self.velocity_thresholds[1])
