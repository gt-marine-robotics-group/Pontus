...
class PositionNode(Node):
    ...
    def odometry_callback(self, msg):
        ...
        # If we need to move linearly, set the angular error to the angle we need to be to approach the coordinate 
        if np.linalg.norm(linear_err[:2]) > self.thresh:
          angular_err = np.array([0, 0, np.arctan2(linear_err[1], linear_err[0])])
          # We want to go straight, but we need to turn first
          if abs(self.angle_wrap(angular_err[2])) > self.angular_thresh:
